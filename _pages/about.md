---
permalink: /
title: Добро пожаловать на мою страницу
excerpt: "About me"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

Hey! I am Sricharan AR, a second year Masters in Computer Science student from Chennai Mathematical Institute, who also did his Bachelors in Mathematics and Computer Science from CMI. Check out the next section for my academic interests, and the later ones for my non-academic ones.

Academic interests
======

I am primarily interested in topics in the intersection of Mathematics and Computer Science, specifically, in non-increasing order of familiarity, (Algorithmic +) Game Theory, Probability Theory, Algorithms, Complexity Theory, and Games on Graphs. I've mainly worked on fair allocation of indivisible items, and relating completely mixed bimatrix games to completely mixed matrix games, but I would love to explore other areas as well!

Books
======

I, like other kids who grow up without siblings and decide to make "I read books" a personality trait, have a [Goodreads account](https://www.goodreads.com/review/list/8544194?order=d&sort=date_read). I'm always looking for new book recommendations, and if you'd like some fiction suggestions,

* [The Brothers Karamazov](https://www.goodreads.com/book/show/4934.The_Brothers_Karamazov) by Fyodor Dostoyevsky
* [The Melancholy of Resistance](https://www.goodreads.com/book/show/119512.The_Melancholy_of_Resistance) by László Krasznahorkai
* [In Search of Lost Time](https://www.goodreads.com/book/show/18796.In_Search_of_Lost_Time) by Marcel Proust

Linux
======

I currently have an Arch Linux installation on my laptop, with [these dotfiles](https://github.com/SriAR/dotfiles). Some things of interest to you might be my:

* [vimrc](https://github.com/SriAR/dotfiles/blob/master/vim/vimrc) - with comments
* [zinc](https://github.com/SriAR/dotfiles/blob/master/bin/zinc) - a custom file syncer using rsync. I mainly use my university-provided computer space as a cloud for my academic PDFs, and use **zinc** to keep them synced between my laptop, phone, iPad and the computer lab at CMI.
* I also have some tex snippets from [this amazing blog post](https://castel.dev/post/lecture-notes-1) which I adapted to my use case, which I will add to my dotfiles when I get the time to.

Dota2
======

I have spent a small portion of my life playing Dota2, and a smaller portion of it watching the game. I have nothing more to say about it, but it just felt weird not having a section in my homepage not dedicated to this game. If you'd like an introduction to the game, look no further than Purge's [Welcome to Dota, you suck](https://purgegamers.true.io/g/dota-2-guide/#welcome-to-dota-you-suck). The title is amazing, and if I get the time to, I would like to write an article titled "Welcome to envy-free allocations, you suck".
